...
export class ReportingService {
  ...
  async balanceSheet(orgId: string, period?: string) {
    const balances = await this.ledger.balances(orgId, period);
  ...
  async incomeStatement(orgId: string, period?: string) {
    const balances = await this.ledger.balances(orgId, period);
    const accounts = await Promise.resolve(this.store.listAccounts(orgId));
  ...
  async cashFlow(orgId: string, period?: string) {
    const income = await this.incomeStatement(orgId, period);
  ...
  async treasury(orgId: string) {
    const balances = await this.ledger.balances(orgId);
    const accounts = await Promise.resolve(this.store.listAccounts(orgId));
    const wallets = await Promise.resolve(this.store.listWallets(orgId));

    const treasuryAccounts = accounts.filter((a: Account) => a.name.toLowerCase().includes("crypto"));
    const stableAccounts = accounts.filter((a: Account) => a.name.toLowerCase().includes("stable"));

    const getBalance = (acc: Account) => balances.get(acc.id) ?? 0;
    return {
      wallets,
      crypto: treasuryAccounts.map((a: Account) => ({ account: a.name, balance: getBalance(a) })),
      stablecoins: stableAccounts.map((a: Account) => ({ account: a.name, balance: getBalance(a) }))
    };
  }
}
